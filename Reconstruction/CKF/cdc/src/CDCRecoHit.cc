...
genfit::SharedPlanePtr CDCRecoHit::constructPlane(const genfit::StateOnPlane& state) const
{
  ...
  //GeomeryWire::Get()->getWirePos(m_wireID.getILayer(),m_wireID.getIWire(),noSagWire1,noSagWire2);
  GeomeryWire::Get()->getSeg()->cellposition2(0,m_wireID.getILayer(),m_wireID.getIWire(),noSagWire1,noSagWire2);
  //GeomeryWire::Get()->getWirePos(m_wireID.getILayer(),m_wireID.getIWire(),noSagWire1,noSagWire2);
  ...
  TVector3 poca_onwire = noSagWire1+noSagWireDirection*((noSagPoca - noSagWire1)*noSagWireDirection);
  ...
  //GeomeryWire::Get()->getWirePos(m_wireID.getILayer(),m_wireID.getIWire(),wire1,wire2);
  //现在是直丝，所以不需要修改
  //TODO: stero axiel
  //Cal wireID
  int wireID_z=GeomeryWire::Get()->getSeg()->wireID_Z(0,m_wireID.getILayer(),poca_onwire);

  //GeomeryWire::Get()->getSeg()->cellposition2(0,m_wireID.getILayer(),m_wireID.getIWire(),wire1,wire2);
  GeomeryWire::Get()->getSeg()->cellposition2(0,m_wireID.getILayer(),wireID_z,wire1,wire2);
...
//N.B. The folowing 8 lines are tentative to avoid the circular dependence mentioned above ! The definitions of alpha and theta should be identical to those defined in CDCGeometryPar.
  ...
  */
  ...
  double mL = m_tdcCount*40*1e-4;//cm
  //s_tdcCountTranslator->getDriftLength(m_tdcCount, m_wireID, trackTime,
              //                                     false, //left
              //                                     z, alpha, theta, m_adcCount);
  double mR = m_tdcCount*40*1e-4;//cm
  //s_tdcCountTranslator->getDriftLength(m_tdcCount, m_wireID, trackTime,
              //                                     true, //right
              //                                     z, alpha, theta, m_adcCount);
  ...
  if (m_leftRight < 0) {
    ...
    //TODO:几何大小???半宽
    double rMax = 1.5*1.8;//CEPC漂移室单元大小为18mm
