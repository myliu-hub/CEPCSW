...
#include "EWirePositionToCDC.h"
#include "EWirePosition.h"
...
//  double sagCoeff = ignoreWireSag ? 0 : cdcgp.getWireSagCoef(wirePosSet, iCLayer, iWire);
  ...
  GeomeryWire::Get()->getSeg()->cellposition2(0,iCLayer,iWire,Wstart,Wend);
  Vector3D forwardPos{Wstart};
  Vector3D backwardPos{Wend};
  double sagCoeff = 0;//ignoreWireSag ? 0 : cdcgp.getWireSagCoef(wirePosSet, iCLayer, iWire);
