...
CDCWireTopology* CDCWireTopology::instance=nullptr;
CDCWireTopology& CDCWireTopology::getInstance()
{
  // Definition of the singleton instance
  if(nullptr==instance) instance = new CDCWireTopology();
  return *instance;
}
...
void CDCWireTopology::initialize()
{
  ...
  for (size_t iCLayer = 0; iCLayer < 55; ++iCLayer) {
      int wireMax = GeomeryWire::Get()->getSeg()->maxWireID(0,iCLayer);
      for (size_t iWire = 0; iWire < wireMax; ++iWire) {
          m_wires.push_back(CDCWire(WireID(iCLayer, iWire)));
      }
  }
