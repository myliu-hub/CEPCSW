...
namespace Belle2 {
  ...
  public:
    ...
    void beginEvent() override
    {
      ...
      if (m_param_writeOutDirection == TrackFindingCDC::EForwardBackward::c_Forward) {
        ...
        doForward = false;
          ...
          std::cout << " CDCCKFStateCreator: No valid direction specified. Please use forward/backward." << std::endl;
               ...
               const std::vector<const TrackFindingCDC::CDCWireHit*>& wireHits) override
    {
      ...
      // Create cache over wirehits, if empty:
        ...
        for (size_t i = 0; i < nHits; i++) {
            ...
            //if(path.size()>1){
            //    const auto& last2ndState = path.end()[-2];
            //    if((!last2ndState.isSeed()) || (! last2ndState.getWireHit())){
            //        int last2ndICLayer = last2ndState.getWireHit()->getWire().getICLayer();
            //        if(std::abs(last2ndICLayer-iCLayer)>m_maximalLayerJump2nd){
            //            continue;
            //        }
            //    }
            //}
            ...
            if (! lastState.isSeed()) {
                //deltaPhi:径迹上最后的hit与候选击中单元之间的phi
    ...
    //double m_maximalDeltaPhi =  TMath::Pi() / 93;
    //TODO:myliu:对于不同的动量可能会不一样，低动量deltaPhi可能会比较大
