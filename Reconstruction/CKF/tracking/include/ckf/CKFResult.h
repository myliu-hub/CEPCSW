        ...
        m_mSoP(mSoP)
    {
      ...
      for (const TrackFindingCDC::WithWeight<const AState*> state : path) {
        ...
        if (state->isFitted()) {
          ...
          //if (stateChi2 > m_maximalChi2 or std::isnan(m_maximalChi2) or std::isinf(m_maximalChi2)) {
          if (stateChi2 > m_maximalChi2 or std::isnan(m_maximalChi2)) {
            m_maximalChi2 = stateChi2;
          }

          //if (stateChi2 < m_minimalChi2 or std::isnan(m_minimalChi2) or std::isinf(m_minimalChi2)) {
